...
import com.cts.composer.TV2DashboardComposer;
import com.cts.inward.dto.CheckerDashboardStatsDTO;
import com.cts.inward.dto.InwardBatchDTO;
import com.cts.inward.service.TV2DashboardService;
import com.cts.inward.service.TV2DashboardServiceImpl;

public class TV2_InwardDashboardComposer extends SelectorComposer<Component> {

    // ── KPI Labels ────────────────────────────────────────────────────────
    @Wire private Label lblPendingBatches;
    ...
    // ── Service ───────────────────────────────────────────────────────────
    private final TV2DashboardService service = new TV2DashboardServiceImpl();
...
//    private final SimpleDateFormat sdfDisplay = new SimpleDateFormat("dd-MMM-yyyy HH:mm");
    ...
    public void doAfterCompose(Component comp) throws Exception {
        ...
        System.out.println("===== TV2 DASHBOARD LOADED =====");
    ...
    private void loadDashboard(Date date) {
        ...
        lblPageMeta.setValue(sdfMeta.format(date) + " · TV2 Queue");

        // Batch list
        currentDateBatches = service.getTV2BatchesForDate(date);
    ...
    private void populateBatchTable(List<InwardBatchDTO> batches) {
        ...
        for (InwardBatchDTO batch : batches) {
            ...
            item.appendChild(new Listcell(
                String.valueOf(batch.getTotalCheques() != null ? batch.getTotalCheques() : 0)));

            // 4. Assigned to TV2 Checker (cheques with checkerQueue = TV2)
            item.appendChild(new Listcell(
                String.valueOf(batch.getAssignedCheques() != null ? batch.getAssignedCheques() : 0)));

            // 5. Pending Cheques
            item.appendChild(new Listcell(
                String.valueOf(batch.getPendingCheques() != null ? batch.getPendingCheques() : 0)));

            // 6. Cleared Cheques
            item.appendChild(new Listcell(
                String.valueOf(batch.getClearedCheques() != null ? batch.getClearedCheques() : 0)));
    ...
    private void redirectToChequeDetails(String batchId) {

        TV2DashboardComposer shell = TV2DashboardComposer.getInstance();

        if (shell == null) {
            Clients.showNotification("Navigation error: shell not found.",
                "error", null, "top_center", 3000);
            return;
        }

        // Store batchId on Desktop so BatchDetailComposer can read it
        Executions.getCurrent().getDesktop().setAttribute("batchId", batchId);
    ...
    private String badgeStyle(String status) {
        if (status == null) return defaultBadge();
